...
function getAncestorHashForBranch() {
  const result = childProcess.spawnSync('git', ['merge-base', 'HEAD', 'origin/master'], {
    encoding: 'utf8',
  });

  if (result.status !== 0) {
    throw new Error('Unable to determine current hash with `git merge-base HEAD origin/master`');
  }
